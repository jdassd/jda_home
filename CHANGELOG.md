...
### 改进
- **用户界面优化**
  - 移除了首页始终显示的用户信息卡片，让界面更简洁
  - 将用户统计信息（使用天数、分类数、链接数）整合到顶部导航栏的用户下拉菜单中
  - 修复了顶部导航栏"个人资料"入口无法正常跳转的问题

### 新增功能
- **分享页面功能**
  - 创建了全新的分享页面 (`/share/:userId`)，采用美观的渐变色设计
  - 支持查看其他用户的公开导航内容
  - 当前登录用户访问自己的分享页时可查看所有内容（包括私有数据）
  - 分享页支持实时搜索功能，可按分类名、链接标题、URL或描述搜索
  - 页面底部展示统计信息（分类数、链接数、公开链接数）
  - 支持一键复制分享链接到剪贴板

- **个人资料管理**
  - 新增个人资料页面 (`/profile`)
  - 支持查看和编辑个人基本信息（用户名、邮箱）
  - 显示账户统计信息（分类数、链接数、使用天数等）
  - 提供修改密码功能（开发中）
  - 提供修改安全问题功能（开发中）
  - 支持账户删除操作（需二次确认）
...
- **首页优化**
  - 个性化欢迎语，显示用户名称
  - 改进了首页布局，移除了冗余的用户信息显示

- **用户信息API**
  - 新增 `/api/auth/users/:userId/info` 公开接口
  - 更新 `/api/auth/me` 接口，返回完整用户信息包括创建时间
  - 返回用户基本信息（用户名、创建时间）
  - 不暴露敏感信息（密码、邮箱等）

### 改进
- **界面优化**
  - 分享页采用紫色渐变头部设计，视觉效果更美观
  - 用户头像显示首字母，增强个性化体验
  - 链接卡片支持hover效果，显示外链图标
  - 完全响应式设计，移动端友好
  - 支持emoji图标和自定义图标显示
  - 首页用户信息卡片采用毛玻璃效果
...
- **用户体验**
  - 在首页添加"我的分享页"快捷入口
  - 未登录用户也可以访问公开的分享页
  - 加载、空状态、错误状态都有友好的提示界面
  - 搜索功能支持实时过滤，无需点击按钮
  - 支持点击外部关闭下拉菜单

### 路由更新
- 新增 `/share/:userId` 路由用于分享页面
- 新增 `/profile` 路由用于个人资料管理（需要登录）
- 更新路由守卫，将分享页设为公开访问

### 数据模型更新
- 前端 User 接口新增 `createdAt` 和 `updatedAt` 字段
- 后端 API 返回完整的用户时间信息
