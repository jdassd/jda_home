...
##### 新增字段
- `users` 表新增 `security_question` 字段（安全问题）
- `users` 表新增 `security_answer` 字段（安全问题答案，加密存储）
...
1. **User 模型** (`backend/src/models/User.ts`)
   - 新增 `securityQuestion` 和 `securityAnswer` 字段
   - 新增 `setSecurityAnswer()` 方法用于加密存储答案
   - 新增 `validateSecurityAnswer()` 方法用于验证答案

2. **认证控制器** (`backend/src/controllers/authController.ts`)
   - 更新 `register` 接口，支持设置安全问题
   - 新增 `getSecurityQuestion` 接口，根据邮箱获取安全问题
   - 新增 `resetPassword` 接口，验证安全问题答案后重置密码

3. **路由配置** (`backend/src/routes/authRoutes.ts`)
   - 新增 POST `/api/auth/security-question` - 获取安全问题
   - 新增 POST `/api/auth/reset-password` - 重置密码

4. **数据库迁移脚本** (`backend/src/migrations/add-security-fields.ts`)
   - 安全地为现有数据库添加新字段
   - 不会删除现有数据
   - 支持重复执行（幂等性）

##### 前端更新
1. **注册页面** (`frontend/src/pages/LoginView.vue`)
   - 添加安全问题选择下拉框（包含6个常见问题）
   - 支持自定义安全问题
   - 添加安全问题答案输入框
   - 答案不区分大小写，自动去除首尾空格

2. **忘记密码页面** (`frontend/src/pages/ForgotPasswordView.vue`)
   - 实现三步式密码重置流程
   - 步骤1：输入邮箱地址
   - 步骤2：回答安全问题
   - 步骤3：设置新密码
...
##### 新用户注册
1. 在注册表单中填写基本信息（用户名、邮箱、密码）
2. 选择一个安全问题（或选择"自定义问题"并输入）
3. 输入安全问题的答案（不区分大小写）
4. 完成注册
...
- 暂时移除了夜间模式切换功能
- 保留夜间模式图标但设为禁用状态
- 点击图标显示"夜间模式正在开发中，敬请期待！"提示

#### 技术细节

##### 前端更新
1. **主布局组件** (`frontend/src/layouts/MainLayout.vue`)
   - 移除了 `isDark` 状态变量
   - 移除了 `toggleTheme` 切换函数
   - 添加了 `showThemeMessage` 函数显示开发中提示
   - 主题切换按钮添加了 `theme-toggle-disabled` 样式类
   - 清理了 localStorage 中的主题设置
   - 移除了暗色主题下的特定样式

2. **全局样式** (`frontend/src/styles/global.css`)
   - 注释了夜间模式的CSS变量定义
   - 保留代码以便后续开发时重新启用
   - 保持界面使用浅色主题

#### 用户体验
- 用户点击夜间模式按钮时会看到友好的提示信息
- 图标呈现半透明禁用状态，不再有旋转动画
- 避免了未完成功能给用户带来的困扰
